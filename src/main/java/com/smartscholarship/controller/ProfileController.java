...
public class ProfileController {
    ...
    public boolean handleSubmit(
        ...
        if (name.isEmpty()
                ...
                || gpa.isEmpty()
                || income.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Incomplete Profile", "Please complete all profile fields.");
            return false;
        }
        if (!privacyCheckBox.isSelected()) {
            showAlert(Alert.AlertType.WARNING, "Privacy Policy Required", "Please agree to the Privacy Policy before submitting.");
            return false;
        }
        try {
            double gpaValue = Double.parseDouble(gpa);
            if (gpaValue < 0.0 || gpaValue > 4.0) {
                showAlert(Alert.AlertType.WARNING, "Invalid GPA", "Please enter a GPA between 0.0 and 4.0.");
                return false;
            }
        } catch (NumberFormatException e) {
            showAlert(Alert.AlertType.WARNING, "Invalid GPA", "Please enter a valid numeric GPA.");
            return false;
        }
        try {
            double incomeValue = Double.parseDouble(income);
            if (incomeValue < 0) {
                showAlert(Alert.AlertType.WARNING, "Invalid Income", "Household income cannot be negative.");
                return false;
            }
        } catch (NumberFormatException e) {
            showAlert(Alert.AlertType.WARNING, "Invalid Income", "Please enter a valid annual household income.");
            return false;
        }
        ...
        showAlert(Alert.AlertType.INFORMATION, "Profile Submitted", "Your profile has been submitted successfully.");
        return true;
    }
